function ExportToMarkdown() {
	...
	this.formatMarkdown = function ({ title, content }) {
		let filename = title;
		let markdown = [];
		...
		//Maps an html tag to the corresponding md and pushes to markdown array
		...
		const downloadFile = (filename, data) => {
			...
			if (images.length > 0) {
				...
				zip
					...
					.catch(console.log("Error occured trying to zip the files"));
			} else {
		...
		let arrayOfElements = [...elements];
		arrayOfElements.forEach((element) => {
			let mdOfElement = extractChildren(element);
